...
class _ChooseLocationState extends State<ChooseLocation> {
  ...
  //overriding the original initState function from the State class

  /*add a state like(for ex: counter integer) and change it, so that we can see that
  every time we call the setState and change the state, we trigger the build function
   */
  int counter=0;
  @override
  void initState() {
    ...
    super.initState();      //this is the original inherited init function
                            //write the code to be executed after running init function
...
// manage as even home page can be stacked into the tree(redundant pages stacked)
*/
...
/*
//below request depends upon the execution of first, then this kind of code won't work
//because we are starting second before first is completed
//Solution is to attach async after parenthesis and before curly braces and put await keyword(in the function name)
    //the below line means: wait right here until its done, to start next line
...
*/
    ...
    print('$username - $bio');
    //we are waiting for each request to move on(only good when one request on other)
  }
  @override
  void initState() {
    super.initState();
  ...
  */
  ...
  void updateTime(index) async{
    ...
    await instance.getTime();     //getTime is async and hence we add await, as we will wait for it to complete
    //navigate to home screen
    //to reroute we used navigator.pushed name . Now we don't want to do this right now
    //we want to pop this one back off

    Navigator.pop(context, {    //pop has a diff syntax, no need to write argument keyword
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView.builder(
        ...
        itemBuilder: (context, index){    //will return widget template
          return Padding(
            padding: const EdgeInsets.symmetric(vertical: 1.0, horizontal: 4.0),
            child: Card(
              child: ListTile(             //see in documentation, image on left and text on right
                onTap: (){
                  updateTime(index);
                },
                title: Text(locations[index].location),
                leading: CircleAvatar(      //to output image next to each one
